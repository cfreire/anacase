...
def setup_logger(log_file=_logfile_):
    ...
    logging.info(f"starting logger on {mac}")
...
def get_random_samples(random_param):
    ...
    case_random = []
    percentage_sample = random_param['percentage_sample']
    loop_sample = random_param['loop_sample']
    logging.info(f'starting generate random percentage {percentage_sample} of {loop_sample}...')
    try:
